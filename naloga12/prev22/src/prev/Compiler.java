...
//						System.out.println("EXIT CODE: " + interpreter.run("_main"));
					...
					// Register allocation.
					HashMap<MemTemp, Integer> tempRegsFromRegAll = null;
					try (RegAll regall = new RegAll(Integer.decode(cmdLine.get("--nregs")))) {
						regall.allocate();
						regall.log();
						tempRegsFromRegAll = regall.tempToReg;
					}
					if (Compiler.cmdLineArgValue("--target-phase").equals("regall"))
						break;
					// The last phase - FINAL PHASE
					try (All finalPhase = new All(tempRegsFromRegAll, Integer.decode(cmdLine.get("--nregs")))) {
						finalPhase.compile();
					}
